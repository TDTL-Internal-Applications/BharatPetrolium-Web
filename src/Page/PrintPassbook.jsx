...
//   const [passbookType, setPassbookType] = useState("RD Passbook");
...
export default function PrintPassbook() {
  ...
  useEffect(() => {
    if (memberId && passbookType) {
      fetchMemberInfo(); // Fetch member details for front page
      if (passbookType === "RD Passbook") {
        fetchRDTransactions(); // Fetch transactions for back page
      } else if (passbookType === "Savings Passbook") {
        fetchSavingsTransactions(); // Fetch transactions for back page
      }
    }
  }, [memberId, passbookType]);
