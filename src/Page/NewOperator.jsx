...
export default function NewOperator() {
  ...
  useEffect(() => {
    const fetchPositionFromApi = async () => {
      try {
        const response = await axios.post(
          "http://bpcl.kolhapurdakshin.com:8000/fetch_role_id/",
          {
            type: selectedOption,
          }
        );
    ...
    if (selectedOption) {
      fetchPositionFromApi();
    } else {
      setCheckboxVisible(false); // Hide the checkbox if no option is selected
    }
  }, [selectedOption]);
    ...
    <>
      ...
      <div className="container-fluid dashboard-area d-flex">
        <div className="main-content p-4">
          <Header />
          <div className="container d-flex justify-content-start w-100 pb-1">
          ...
          <div className="container d-flex py-3">
            <div className="row w-100">
              ...
              <div className="py-2">
                {checkboxVisible && (
                  <div>
                    <input type="checkbox" value={position} checked={position !== ''} /><span style={{paddingLeft:"7px"}}>{position}</span>
                  </div>
                )}
              </div>
            </div>
